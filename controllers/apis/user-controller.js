...
const userController = {
  ...
  likeComment: (req, res, next) => {
    userServices.likeComment(req, (err, data) => err ? next(err) : res.json({ status: 'success', data }))
  },
  unlikeComment: (req, res, next) => {
    userServices.unlikeComment(req, (err, data) => err ? next(err) : res.json({ status: 'success', data }))
  }
}
