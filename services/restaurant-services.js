const { Restaurant, Category, User, Comment, LikeComment } = require('../models')
  ...
  getRestaurants: (req, callback) => {
    ...
    return Promise.all([
      ...
      .then(([restaurants, categories]) => {
        ...
        const likedRestaurantsId = req.user?.LikedRestaurants ? req.user.LikedRestaurants.map(liked => liked.id) : []
  ...
  getDashboard: (req, cb) => {
    return Restaurant.findByPk(req.params.id, {
      ...
      .then(r => {
        ...
        return cb(null, { restaurant, viewCounts })
      })
      .catch(err => cb(err))
  },
  getFeeds: (req, cb) => {
